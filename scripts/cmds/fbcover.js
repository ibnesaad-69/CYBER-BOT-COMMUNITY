...
if ((this.config.credits) != "Abdulla Rahaman") { return api.sendMessage(`[ WARNING ] - Abdulla Rahaman Don't credits modules ${this.config.name} was changed to ${this.config.credits} bởi ADMINBOT ${global.config.BOTNAME} 😐 Stop now!!!`, event.threadID, event.messageID)}
  ...
  if(!args[0]) return api.sendMessage('Hey Itz Me 𝗖𝗬𝗕𝗘𝗥-𝗖𝗔𝗧 here Please enter primary name!!!', threadID, messageID)
  else return api.sendMessage(`𝗖𝗬𝗕𝗘𝗥-𝗖𝗔𝗧 😍 You chose the main name: ${args.join(" ").toUpperCase()}\n\n(Reply to this message and choose your secondary name)`,event.threadID, (err, info) => {
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  switch (handleReply.type) {
    ...
    case "sdt": {
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(`𝗖𝗬𝗕𝗘𝗥-𝗖𝗔𝗧 You have selected SDT as: ${event.body.toUpperCase()}\n\(Reply this message to enter your email)`,threadID, function (err, info) {
        return global.client.handleReply.push({
          type: "email",
          name: `fbcover`,
          author: senderID,
    ...
    case "email": {
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(`𝗖𝗬𝗕𝗘𝗥-𝗖𝗔𝗧 😘 You have selected email as : ${event.body.toUpperCase()}\n\(Reply this message to enter your address)`,threadID, function (err, info) {
        return global.client.handleReply.push({
          type: "color",
          name: `fbcover`,
          author: senderID,
    ...
    case "create": {
      ...
      api.unsendMessage(handleReply.messageID);
      api.sendMessage(`Hey Itz Me 𝗖𝗬𝗕𝗘𝗥-𝗖𝗔𝗧 Your Cover Almost Done Please Wait⏳ Initializing the imager!`,threadID, (err, info) => {
      setTimeout(() => {
